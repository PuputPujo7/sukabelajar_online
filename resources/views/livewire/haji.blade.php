<section class="container mx-auto pt-5 justify-center z-0 w-full top-12">
    <div class="mx-auto w-full">
        <h1 class="mx-auto text-3xl py-4 border-b mb-5 bg-pink-50 rounded-lg text-center w-auto font-semibold">MATEMATIKA</h1>


        <div class="flex bg-pink-50 rounded-lg shadow relative w-full px-3 py-3 justify-center items-center">
            @foreach ($kategoris as $kategori)
            {{-- <button wire:click="modal({{$kategori->id}})"
                ...
                <a href="{{route('detail',$kategori->id)}}" class="elipsis">
                <div
                    class="flex shadow rounded mr-2 ml-2 mb-2 hover:scale-110 transition duration-300 ease-in-out">
                    <div class="w-full p-1 md:p-2">
                        <img alt="kategori" class="block object-cover object-center rounded-lg " style="height: 200px"
                            {{-- src="{{ asset('storage/' . $kategori->kategori) }}"> --}} src="{{ asset('storage/'.$kategori->gambar) }}">
                    </div>
